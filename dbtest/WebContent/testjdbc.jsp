		...
		try {
			if (className != null && url != null) {
				...
				if (!"".equals(sqlQuery) && sqlQuery != null){
					...
					log.info("准备执行SQL：" + t3);
					pst = conn.prepareStatement(sqlQuery);	//准备执行语句  
					rs = pst.executeQuery();
					...
					log.info("getColumnName:"+ c1 +","+c2);
					out.println("getColumnName:"+ c1 +","+c2+"<br/>");
					while(rs.next()){
						double s1 = rs.getDouble(1);
						String s2 = rs.getString(2);
						log.info("rsCount:"+ rsCount + ","+ s1 +","+s2);
						out.println("rsCount:"+ rsCount + ","+ s1 +","+s2+"<br/>");
						rsCount++;
					}
...
<body >
<h3>数据库JDBC驱动创建和查询测试</h3>
<form action="testjdbc.jsp" method="post">
<table border="1">
<tr>
